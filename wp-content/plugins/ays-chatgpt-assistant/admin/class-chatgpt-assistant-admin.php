    ...
    */
	...
	public function set_global_settings(){
        ...
        // * STYLE SETTINGS *
			...
			$this->chatgpt_assistant_send_message_icon_text = __('Upload Image', $this->plugin_name);
			if ( $this->chatgpt_assistant_send_message_icon != '' ) {
				$this->chatgpt_assistant_send_message_icon_text = __('Edit Image', $this->plugin_name);
			}
            ...
            $content[] = '<div id="ays-chatgpt-new-mega-bundle-dicount-month-main-2024" class="notice notice-success is-dismissible ays_chatgpt_dicount_info">';
                $content[] = '<div id="ays-chatgpt-dicount-month" class="ays_chatgpt_dicount_month">';
                    ...
                    $content[] = '<div class="ays-chatgpt-dicount-wrap-box ays-chatgpt-dicount-wrap-button-box">';
                        $content[] = '<a href="https://ays-pro.com/wordpress/chatgpt-assistant?utm_source=dashboard&utm_medium=gpt-free&utm_campaign=sale-banner" class="button button-primary ays-button" id="ays-button-top-buy-now" target="_blank">' . __( 'Buy Now !', CHATGPT_ASSISTANT_NAME ) . '</a>';
        ...
        if(isset($_REQUEST['page'])){
            if(false !== strpos( sanitize_text_field( $_REQUEST['page'] ), $this->plugin_name)){
                ...
                <div class="ays-chatgpt-assistant-footer-support-box">
                    <span class="ays-chatgpt-assistant-footer-link-row"><a href="https://wordpress.org/support/plugin/ays-chatgpt-assistant/" target="_blank"><?php echo __( "Support", "ays-chatgpt-assistant"); ?></a></span>
                    <span class="ays-chatgpt-assistant-footer-slash-row">/</span>
                    <span class="ays-chatgpt-assistant-footer-link-row"><a href="https://ays-pro.com/wordpress-chatgpt-assistant-user-manual" target="_blank"><?php echo __( "Docs", "ays-chatgpt-assistant"); ?></a></span>
                    <span class="ays-chatgpt-assistant-footer-slash-row">/</span>
                    <span class="ays-chatgpt-assistant-footer-link-row"><a href="https://ays-demo.com/chatgpt-ai-assistant-plugin-feedback-survey/" target="_blank"><?php echo __( "Suggest a Feature", "ays-chatgpt-assistant"); ?></a></span>
                </div>
    ...
    public function ays_chatgpt_connect(){
        if (!wp_verify_nonce($_REQUEST['nonce'], 'ajax-nonce') || !current_user_can('manage_options')) {
    ...
    public function ays_chatgpt_save_wp_media() {
		$image_url = isset($_REQUEST['image_url']) ? esc_url_raw($_REQUEST['image_url']) : '';
		...
		if (!filter_var($image_url, FILTER_VALIDATE_URL)) {
			wp_send_json(['status' => false, 'message' => __('Invalid image URL provided', 'chatgpt-assistant')]);
			return;
		}
		...
		if (is_wp_error($tmp)) {
			wp_send_json(['status' => false, 'message' => __('Error downloading the image:', 'chatgpt-assistant') . $tmp->get_error_message()]);
			return;
		}
		...
		if (is_wp_error($media_id)) {
			wp_send_json(['status' => false, 'message' => __('Error saving the image:', 'chatgpt-assistant') . $media_id->get_error_message()]);
			return;
		}
	
		$image_url = wp_get_attachment_url($media_id);
		wp_send_json(['status' => true, 'message' => __('Image saved to WordPress media library. URL: ', 'chatgpt-assistant') . $image_url]);
	}
    ...
    public function ays_chatgpt_make_request() {
        ...
        if ( !isset($apiKey) || $apiKey === "" ) {
            wp_send_json_error(['message' => 'Invalid API Key.', 'error' => []]);
        }
